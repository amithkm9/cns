...
def main():
    ...
    if 'pharmacy_user' not in st.session_state:
        st.session_state.pharmacy_user = None
...
def show_doctor_portal():
    ...
    if 'pharmacy_list' not in st.session_state:
        # Create some sample pharmacies
        st.session_state.pharmacy_list = {
            "Central Pharmacy": User("Central Pharmacy", "pharmacy"),
            "City Drugs": User("City Drugs", "pharmacy"),
            "Health Pharmacy": User("Health Pharmacy", "pharmacy")
        }
        # Generate keys for each pharmacy
        for pharmacy in st.session_state.pharmacy_list.values():
            pharmacy.generate_key_pair()
    
    selected_pharmacy = st.selectbox(
        "Select a pharmacy",
        list(st.session_state.pharmacy_list.keys())
    )
    ...
    if st.button("Create and Encrypt Prescription"):
        with st.spinner("Creating and encrypting prescription..."):
            ...
            selected_pharmacy_user = st.session_state.pharmacy_list[selected_pharmacy]
...
def show_pharmacy_portal():
    ...
    with st.expander("Pharmacy Credentials", expanded=True):
        ...
        with col1:
            pharmacy_name = st.text_input("Pharmacy Name", value="Central Pharmacy")
        
        with col2:
            if st.button("Generate Key Pair"):
                st.session_state.pharmacy_user = User(pharmacy_name, "pharmacy")
                st.session_state.pharmacy_user.generate_key_pair()
                st.session_state.pharmacy_portal = PharmacyPortal(st.session_state.pharmacy_user)
                st.success(f"Key pair generated for {pharmacy_name}")
    
    # Cannot proceed without pharmacy key pair
    if not st.session_state.pharmacy_user:
        st.warning("Please generate a key pair to continue")
        return
